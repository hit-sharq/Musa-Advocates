...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="en-KE"><body>{children}<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ '@context': 'https://schema.org', '@type': 'LegalService', name: 'Musa & Musa Advocates', url: 'https://musadvocates.co.ke', email: 'info@musadvocates.co.ke', telephone: '+254758251399', address: { '@type': 'PostalAddress', streetAddress: '6076 Rhapta Road', addressLocality: 'Nairobi', addressCountry: 'KE' }, areaServed: 'Kenya' }) }} />{process.env.NODE_ENV === 'production' && <Analytics />}</body></html>
}
